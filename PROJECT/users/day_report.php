...
@$d=$_POST["d"];
      ...
      if(isset($go))
    {
      if($d!='')
      {
        ...
        <h5 style="color: ">REPORT  ON DATE <?php echo " ".$d  ?> </h5>
...
$sql = "SELECT customers.fname,customers.lname,orders.time,food.quantity,categories.name,
orders.id,orders.status FROM categories,food,orders,customers 
where customers.id=orders.cid and orders.fid=food.id and food.cid=categories.id
 and orders.cid=customers.id and orders.date='$d'";
      ...
      <h5 class="card-title" style="color:rgb(90, 108, 188)" >There is no order in at this date </h5>
    </div></div>
